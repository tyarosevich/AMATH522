function dy = repress(t,y,p)
 
   dy = zeros(12,1);
   dy(1) = -y(1) + p(1)/(1.+y(6)^p(4))+ p(2);
   dy(2) = -y(2) + p(1)/(1.+y(4)^p(4))+ p(2);
   dy(3) = -y(3) + p(1)/(1.+y(5)^p(4))+ p(2);
   dy(4) = -p(3)*(y(4)-y(1)) + p(5)*(y(10) - y(4));
   dy(5) = -p(3)*(y(5)-y(2))+ p(5)*(y(11) - y(5));
   dy(6) = -p(3)*(y(6)-y(3))+ p(5)*(y(12) - y(6));
   dy(7) = -y(7) + p(1)/(1.+ y(12)^p(4)) + p(2); 
   dy(8) = -y(8) + p(1)/(1.+ y(10)^p(4)) + p(2); 
   dy(9) = -y(9) + p(1)/(1.+ y(11)^p(4)) + p(2); 
   dy(10)= -p(3)*( y(10) - y(7)) + p(5)*(y(4) - y(10));
   dy(11)= -p(3)*( y(11) - y(8)) + p(5)*(y(5) - y(11));
   dy(12)= -p(3)*( y(12) - y(9)) + p(5)*(y(6) - y(12));


