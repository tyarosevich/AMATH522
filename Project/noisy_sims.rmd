...
noisy_SEIR <- function(t, x, parms) {
  with(as.list(c(parms,x)),{
    if(t < tau1) beta <- beta0
    else beta <- beta0*exp(-k*(t-tau1))
    random = c(rnorm(5))
    N <- S + E + I + R
    dS <- - beta*S*I/N + S*rnorm[2]*.1
    dE <- beta*S*I/N - sigma*E + E*rnorm[3]*.1
    dI <- sigma*E - gamma*I + I*rnorm[4]*.1
    dR <- (1-f)*gamma*I + R*rnorm[5]*.1
    dD <- f*gamma*I
    dC <- sigma*E
...
nll <- function(beta0,k,f,tau0,tau1,sigma,gamma) {
  ...
  simulation <- as.data.frame(ode(init,times,noisy_SEIR,parms=pars, method = "euler"))
